...
import { Plus, History, Trash2, Clock, BarChart3, Droplets, GlassWater } from 'lucide-react';
          ...
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                className="bg-orange-50/50 p-4 rounded-2xl border border-orange-100/50 text-center hover:bg-orange-50 transition-all group cursor-pointer"
              >
                <div className="w-10 h-10 bg-orange-500 rounded-full flex items-center justify-center mx-auto mb-2 text-white shadow-lg shadow-orange-500/20 group-hover:scale-105 transition-transform text-lg">
                  🍼
                </div>
                <span className="text-xs font-bold text-orange-950">Ăn uống</span>
              </button>

              <button 
                onClick={() => setLogModal({ isOpen: true, type: 'sleep' })}
                className="bg-purple-50/50 p-4 rounded-2xl border border-purple-100/50 text-center hover:bg-purple-50 transition-all group cursor-pointer"
              >
                <div className="w-10 h-10 bg-purple-500 rounded-full flex items-center justify-center mx-auto mb-2 text-white shadow-lg shadow-purple-500/20 group-hover:scale-105 transition-transform text-lg">
                  💤
                </div>
                <span className="text-xs font-bold text-purple-950">Ngủ nghỉ</span>
              </button>

              <button 
                onClick={() => setLogModal({ isOpen: true, type: 'diaper' })}
                className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 text-center hover:bg-blue-50 transition-all group cursor-pointer"
              >
                <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center mx-auto mb-2 text-white shadow-lg shadow-blue-500/20 group-hover:scale-105 transition-transform text-lg">
                  💩
                </div>
                <span className="text-xs font-bold text-blue-950">Vệ sinh</span>
              </button>

              <button 
                onClick={() => setIsPumpingOpen(true)}
                className="bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100/50 text-center hover:bg-emerald-50 transition-all group cursor-pointer"
              >
                <div className="w-10 h-10 bg-[#1D9E75] rounded-full flex items-center justify-center mx-auto mb-2 text-white shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform text-base">
                  <GlassWater className="w-5 h-5" />
                </div>
                <span className="text-xs font-bold text-emerald-950">Hút sữa</span>
              </button>
            </div>
          </div>
        </div>

        {/* Right Column: Tabbed Logs, Analytics & Milk Storage */}
